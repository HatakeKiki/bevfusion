...
class ImageAug3D:
    ...
    def sample_augmentation(self, results):
        ...
        resize_W = self.resize_lim[0]
        resize_H = self.resize_lim[1]
        ...
        return self.resize_lim, resize_dims, crop, flip, rotate
    ...
    def img_transform(
        ...
        # img.save('/home/kiki/jq/lss/bevfusion/ori.png')
        img = img.crop(crop)
        if flip:
            img = img.transpose(method=Image.FLIP_LEFT_RIGHT)
        img = img.rotate(rotate)
        # img.save('/home/kiki/jq/lss/bevfusion/new.png')
        img = np.array(img).astype(np.float32)

        # post-homography transformation
        # rotation *= resize
        rotation[0, 0] *= resize[0]
        rotation[1, 1] *= resize[1]
...
class GlobalRotScaleTrans:
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.is_train:
            ...
            trans_std = [self.trans_lim, self.trans_lim, self.trans_lim]
            translation = np.random.normal(scale=trans_std, size=3).T
            rotation = np.eye(3)
