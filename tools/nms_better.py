...
# from mmdet3d.core.post_processing.box3d_nms import box3d_multiclass_nms, aligned_3d_nms, circle_nms
from mmdet3d.core.post_processing import box3d_nms
...
def main():
    ...
    # Merge predictions from all files per frame
    merged_predictions = {}
    ...
    for sample_token, prediction in tqdm(predictions.items()):
        ...
        for name, pred in pred_dicts.items():
            ...
            with torch.no_grad():
                ...
                selected = box3d_nms.box3d_multiclass_nms(boxes_for_nms, top_scores_tensor, 
                    pre_max_size=None,
                    post_max_size=50,  
                    iou_threshold=0.2,
                ).numpy()
